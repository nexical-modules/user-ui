...
export async function signIn(provider: string, options: Record<string, any>) {
    if (provider === 'credentials') {
        ...
        formData.append('email', options.identifier || options.email);
        formData.append('password', options.password);
        formData.append('callbackUrl', options.callbackUrl || window.location.href);
    ...
    if (provider === 'github') {
        ...
        callbackInput.value = options.callbackUrl || window.location.href;
