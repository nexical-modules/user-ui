    ...
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            className="admin-action-button h-8 w-8 p-0 border-muted-foreground/20 hover:bg-muted"
            data-testid="admin-actions-trigger"
          >
            <span className="admin-reader-only">
              {t('user.admin.user_management.actions.open_menu')}
            </span>
        ...
        <DropdownMenuContent align="end" className="admin-dropdown-content p-2">
          <DropdownMenuLabel className="mb-2 px-2">
            {t('user.admin.user_management.actions.label')}
          </DropdownMenuLabel>
          <DropdownMenuItem
          ...
          >
            {user.status === UserStatus.ACTIVE ? (
              <>
                <UserX className="admin-icon-left h-4 w-4" />{' '}
                {t('user.admin.user_management.actions.deactivate')}
              </>
            ) : (
              <>
                <UserCheck className="admin-icon-left h-4 w-4" />{' '}
                {t('user.admin.user_management.actions.reactivate')}
              </>
            )}
          </DropdownMenuItem>
          <DropdownMenuSeparator />
